...
class WaterfallLike extends Component {
    state: {
        dataList: item,
        refreshState: number,
    }
    ...
    onHeaderRefresh = () => {
        ...
        const url = name ? `http://127.0.0.1:3000/search/0/${limit}/hot/${name}` : `http://127.0.0.1:3000/search/0/${limit}/hot/`
        fetch(url, {
            method: 'GET',
    ...
    onFooterRefresh = () => {
        ...
        const name = global.storage.name
        const url = name ? `http://127.0.0.1:3000/search/${this.state.offset}/${limit}/hot/${name}` : `http://127.0.0.1:3000/search/${this.state.offset}/${limit}/hot/`
        fetch(url, {
            method: 'GET',
            headers: {
                Accept: 'application/json',
    ...
    cb = () => {
        const name = global.storage.name
        const url = name ? `http://127.0.0.1:3000/search/0/10/hot/${name}` : `http://127.0.0.1:3000/search/0/10/hot`
        fetch(url, {
            method: 'GET',
        }).then(response => response.json()).catch(err => {
            alert(JSON.stringify(err))
